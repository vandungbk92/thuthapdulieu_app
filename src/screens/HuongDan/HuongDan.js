...
import { getAll } from '../../epics-reducers/services/huongdanServices';

import { CHI_TIET_CANH_BAO_PAGE } from '../../constants/router';
...
export default class HuongDan extends React.Component {
  ...
  renderItem = ({ item }) => {
    let uri = item.avatar ? COMMON_APP.HOST_API + API.IMAGES.format(item.avatar) : ''
    return <TouchableOpacity
      style={[tw.flexRow]}
      onPress={() => {
        this.props.navigation.navigate(CHI_TIET_CANH_BAO_PAGE, { info_detail: item });
      }}
    >
      <Image
