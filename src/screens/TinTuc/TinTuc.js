...
import {getAll} from '../../epics-reducers/services/tintucatttServices';

import {CHI_TIET_TIN_TUC_PAGE} from '../../constants/router';
...
export default class TinTuc extends React.Component {
  ...
  renderItem = ({item}) => {
    let uri = item.images ? COMMON_APP.HOST_API + API.IMAGES.format(item.images) : ''
    return <TouchableOpacity style={[tw.flexRow]} onPress={() => {
      this.props.navigation.navigate(CHI_TIET_TIN_TUC_PAGE, {info_detail: item});
    }}>
      <Image
