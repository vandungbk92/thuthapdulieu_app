...
import { getAllRequest } from "../../epics-reducers/services/requestServices";
...
import { REQUEST_DETAIL_PAGE, SEARCH_PAGE } from "../../constants/router";
...
class Requests extends Component {
  static navigationOptions = ({ navigation }) => {
    return {
      ...
      headerTitle: () => (
        <RkText rkType="header4" style={styleContainer.headerTitle}>{I18n.t("recent")}</RkText>
      ),
      headerRight: () => (
        <TouchableOpacity style={styleContainer.headerButton}
          onPress={() => navigation.navigate(SEARCH_PAGE, {
            type: CONSTANTS.REQUESTS,
            onGoBack: navigation.state.params.onPressSearch
          })}>
  ...
  async componentDidMount() {
    try {
      ...
      let apiRequest = [getAllRequest(pageActive)]
      let requests
  ...
  async fetchMore() {
    ...
    let requests = await getAllRequest(pageActive + 1, 10, queryStr);
    if (requests) {
  ...
  async onPressSearch(serviceSelected, districtSelected, statusSelected, startDateReal, endDateReal, searchTitle) {
    ...
    let titleQuery = searchTitle ? `${'&title[like]='}${searchTitle}` : ''
    let queryStr = `${serviceActive ? "&service_id={0}".format(serviceActive) : ""}${statusQuery}${districtActive ? "&district_id={0}".format(districtActive) : ""}${startDate ? "&created_at[from]={0}".format(startDate) : ""}${endDate ? "&created_at[to]={0}".format(endDate) : ""}${titleQuery}`
    let requests = await getAllRequest(pageActive, 10, queryStr);
    if (requests) {
      this.setState({
        serviceSelected,
        districtSelected,
  ...
  onRefresh() {
    ...
    this.props.navigation.navigate(REQUEST_DETAIL_PAGE, {
      id: id,
      type: CONSTANTS.VIEW,
      onGoBack: this.onPressSearch
    })
  }

  render() {
    let checkData = this.state.data ? !!this.state.data.length : null
    return (
      <View style={styleContainer.containerContent}>
        ...
        {checkData ?
          <FlatList
            ...
            renderFooter={() => {
              return (
                this.state.page <= this.state.pages && this.state.isLoadingMore &&
...
function mapStateToProps(state) {
  const { loginRes, setting } = state
  return { loginRes, setting }
}
